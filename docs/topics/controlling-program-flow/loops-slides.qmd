...
Loops are used to execute a block of code multiple times. Python provides two types of loops: `for` and `while`.
...
digraph G {
    ...
    node [shape=box];
    enter [label="Enter the while loop"];
    exit [label="Exit the while loop"];
    body [label="while loop\nbody"];
    
    node [shape=diamond];
    condition [label="Test Condition"];
    
    enter -> condition;
    condition -> exit [label="False"];
    condition -> body [label="True"];
    body -> condition:w [dir=back];
    
    { rank=same; condition exit }
