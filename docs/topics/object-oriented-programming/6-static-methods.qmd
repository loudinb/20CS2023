...
- Inherited like regular methods
- Can be overridden in subclasses
- Don't have access to class-specific data
...
::: {.notes}
Static methods are inherited like regular methods and can be overridden in subclasses. However, they don't have access to class-specific data, so they behave the same regardless of which class they're called on. In this example, `Dog` inherits the `breathe` static method from `Animal` and defines its own `bark` static method.
:::
...
- Inherited but don't have access to class-specific data
